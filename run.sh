...
if [ ! -n "$WERCKER_HIPCHAT_NOTIFY_FAILED_MESSAGE" ]; then
	if [ ! -n "$DEPLOY" ]; then
   		export WERCKER_HIPCHAT_NOTIFY_FAILED_MESSAGE="$WERCKER_APPLICATION_NAME $WERCKER_GIT_BRANCH <a href='"$WERCKER_BUILD_URL"'>build</a> failed (<a href='"https://github.com/RUNDSP/run_portal/commit/$WERCKER_GIT_COMMIT"'>$WERCKER_GIT_COMMIT</a> by $WERCKER_STARTED_BY)"
	else
...
if [ ! -n "$WERCKER_HIPCHAT_NOTIFY_PASSED_MESSAGE" ]; then
	if [ ! -n "$DEPLOY" ]; then
   		export WERCKER_HIPCHAT_NOTIFY_FAILED_MESSAGE="$WERCKER_APPLICATION_NAME $WERCKER_GIT_BRANCH <a href='"$WERCKER_BUILD_URL"'>build</a> passed (<a href='"https://github.com/RUNDSP/run_portal/commit/$WERCKER_GIT_COMMIT"'>$WERCKER_GIT_COMMIT</a> by $WERCKER_STARTED_BY)"
	else
